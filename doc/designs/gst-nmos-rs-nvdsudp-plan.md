...
|---|---|---|
| Sender | `local-iface-ip` / `ptp-src` from `a=x-nvnmos-iface-ip`, else SSM `a=source-filter` include source-address; unset when SDP declares PTP but neither is present | Matches libnvnmos IS-05 `source_ip` and `nvds_nmos_bin` sender wiring |
| Receiver | `use-rtp-timestamp=true`, `adjust-leap-seconds=true`; `local-iface-ip` / `ptp-src` from `a=x-nvnmos-iface-ip` only (not SSM source-filter source) | Aligns with DeepStream recv + `nvdsnmosbin` |
...
| 2 | `ptp-src` default on sender and receiver | **Derive from `a=ts-refclk:` in the effective SDP** (activation file / configuring transport file; on the sender, the copy written to `nvdsudpsink.sdp-file`), not left blindly unset. When the SDP declares PTP (`a=ts-refclk:ptp=…` — traceable, GMID, domain; RFC 7273 clock identity, not a bind address), set `ptp-src` on both `nvdsudpsink` and `nvdsudpsrc` to `local-iface-ip` / `interface_ip`. Mirror `nvds_nmos_bin`'s `gstnvdssdpsink.cpp` / `gstnvdssdpsrc.cpp` (`ptp-src` preset that is not itself a literal IP → fall back to egress/join NIC). When the SDP uses `localmac:` or omits `ts-refclk` entirely, leave `ptp-src` unset (system clock). User overrides via `transport-properties` still win (applied last). | **Done** (`nvdsudp::ts_refclk::ptp_src_from_sdp` in `build_nvdsudpsink` / `build_nvdsudpsrc`) |
