...
service NvnmosDaemon {
  // Sessions. A session contributes resources to exactly one NMOS Node.
  // Many sessions across many client processes may share a Node by node_seed.
  ...
  // the NMOS /senders/<UUID> or /receivers/<UUID> resource id from it.
  // ResourceHandle is the daemon's local handle, not the NMOS UUID.
...
The daemon hosts zero or more NvNmos node-server instances, each with its own HTTP port. A session attaches to a particular Node by the NvNmos seed `node_seed`; the first session for an absent seed creates the Node from a `NodeConfig` payload. The seed is the daemon's lookup key — the underlying NMOS `/self` UUID is generated by NvNmos and is not used as the lookup key. Nodes are torn down when the last contributing session closes (unless `persistent=true`). The common deployment has a single Node; multi-Node is supported but rarely needed in practice.
