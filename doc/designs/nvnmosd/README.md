...
| `mxl-flow-id` | flow_def top-level `id` (sender side; receiver side comes from activation) | Optional; omitted means the Sender is unconstrained | `nmossink` (MXL) |
...
| `receiver-caps-mode` | `auto` (default) / `constrained` / `unconstrained` — controls whether IS-04 publishes constrained Receiver Caps derived from the transport file, or none (`x-nvnmos-caps` / `urn:x-nvnmos:tag:caps` present). `auto` leaves the marker untouched in the spliced transport file, so the result is constrained when the marker is absent and unconstrained when present; `constrained` and `unconstrained` force the marker out of (or into) the spliced transport file. | Optional | `nmossrc` only |
...
- `receiver-caps-mode=auto` (default): the element leaves the unconstrained marker untouched in the spliced transport file. The outcome therefore depends on the file: constrained when the marker is absent, unconstrained when present.
- `receiver-caps-mode=constrained`: IS-04 publishes constrained Receiver Caps matching the transport file. The element forces the constrained path by removing any unconstrained marker from the spliced transport file and rejects any activation carrying a structurally different transport file.
- `receiver-caps-mode=unconstrained`: IS-04 publishes no Receiver Caps. The element splices `urn:x-nvnmos:tag:caps = [""]` on MXL or `a=x-nvnmos-caps:` on RTP/UDP (libnvnmos: present + non-empty = unconstrained) into the transport file and accepts activations whose essence shape differs within the same format family (no `capssetter` pin on the inner chain; chain swap + downstream flush on live reroute). Validated via the `gst-nmos-rs` demo Node 1 ↔ Node 4 switch (RTP/UDP).
...
> **Superseded in Phase 1.** The `input-selector` + flush-start/stop + pause-top-level-pipeline mechanism described below is the **originally-planned** design and was not adopted as-is. Phase 1 implementation replaced it with the same permanent `identity` anchor + block-probe + chain swap mechanism used on `nmossink`. The (A) / (B) / (C) problem decomposition below remains useful; `nvdsudp` is now wired via that anchor pattern, but Rivermax soak may still surface clock-renegotiation needs — see [*Phase 1 as-built: anchor + block-probe*](#phase-1-as-built-anchor--block-probe) below. The prose below is preserved as historical context.
...
- **(B) Clock renegotiation when the wire source's clock changes.** Doesn't bite in Phase 1 because `mxlsrc` doesn't provide the pipeline clock. `nvdsudpsrc` with `use-rtp-timestamp=true` may need to re-introduce some of the pause-top-level-pipeline machinery from the original design; the anchor pattern composes with that orthogonally. Monitor in Rivermax soak.
