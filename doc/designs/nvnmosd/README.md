...
- **Phase 2 — ST 2110 via nvdsudp** (**software landed** in `gst-nmos-rs`; Rivermax hardware soak outstanding): ST 2110-20 video and ST 2110-30 audio via DeepStream `nvdsudpsrc` / `nvdsudpsink` Mode 3 (`transport=nvdsudp`). Property-route SDP synthesis, `TP=2110TPN`, temp SDP files for `nvdsudpsink`, and IS-05 activation are implemented — see [`gst-nmos-rs-nvdsudp-plan.md`](../gst-nmos-rs-nvdsudp-plan.md). ST 2110-40 ANC (Phase 2.1) and end-to-end `receiver-caps-mode=wide` renegotiation remain follow-ups; ANC depends on the nvdsudp ANC patch in DeepStream.
