...
For MXL Senders, the top-level `id` field of the flow definition (if present, a UUID) constrains the MXL flow identity (the `mxl_flow_id` IS-05 transport parameter) to that value. When `id` is absent or empty, `mxl_flow_id` is unconstrained: a Controller may supply a UUID, or staging `"auto"` generates a new UUID at activation. For MXL Receivers, the MXL flow identity is supplied dynamically through IS-05 Connection Management, so the configuring flow definition's `id` field is ignored. The NMOS Flow ID is always derived from the `seed` and the name (`urn:x-nvnmos:tag:name` value) and is independent of `mxl_flow_id`.
